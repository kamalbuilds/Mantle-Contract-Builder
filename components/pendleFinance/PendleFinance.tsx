...
import { HOSTED_SDK_URL, MANTLE_CHAIN_ID } from "@/config/pendleConfig";
import PendleNode from "./components/PendleNode";
...
const BlockNode = ({ data, isDragging, id }) => {
  ...
  return (
    <div
      className={`${
        data.color
      } text-white p-6 rounded-lg shadow-md cursor-pointer select-none
                flex items-center justify-between border-[1px] transition-colors w-[200px] ${
                  isDragging ? "opacity-70" : ""
                }
                ${isSelected ? "border-white shadow-glow" : data.borderColor} ${
...
function Web3BlocksComponent() {
  ...
  const [pendleMarkets, setPendleMarkets] = useState([]); // Add state for token actions

  const fetchAllMarkets = async () => {
    try {
      const res = await fetch(
        `${HOSTED_SDK_URL}/v1/${MANTLE_CHAIN_ID}/markets/active`
      );
  ...
  useEffect(() => {
    const fetchMarkets = async () => {
      ...
      if (markets) {
        ...
        setPendleMarkets(transformedMarkets);
      }
    };
  ...
  const updateFlowSummary = (sourceId, targetId) => {
    ...
    setFlowSummary((prevSummary) => {
      ...
      if (prevSummary.length === 0) {
        return [{ content: sourceNode.data.content, id: sourceId }, newItem];
      }
          ...
          <div className="p-6 w-72 pt-12 selectable-none cursor-default overflow-y-auto flex-grow">
            ...
            <div className="flex flex-col gap-6">
              {Object.entries(groupedBlocks).map(([category, blocks]) => {
                if (category === "Pendle Market") {
                  return (
                    <div key={category}>
                      <h3 className="text-xs mb-2 text-white/80">{category}</h3>
                      <div className="flex flex-col gap-2">
                        {pendleMarkets.map((block) => (
                          <div
                            key={block.id}
                ...
                flex items-center justify-between border-[1px] ${block.borderColor} ${block.hoverBorderColor} transition-colors`}
                          >
                            ...
                            <span>
                              {new Date(block.expiry).toLocaleDateString()}
                            </span>
                ...
                return (
                  <div key={category}>
                    <h3
                      className="text-xs mb-2 text-white/80"
                      style={{
                        color:
                          blocks.length > 0
                            ? blocks[0].color.replace("bg-", "")
                            : "white",
                      }}
                    >
                      {category}
...
const PendleFinanceBlockComponentWrapper = () => (
  <ReactFlowProvider>
    <Web3BlocksComponent />
  </ReactFlowProvider>
);

export default PendleFinanceBlockComponentWrapper;
